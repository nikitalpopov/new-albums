"""
pygn.py :: https://github.com/cweichen/pygn
...
def createRadio(clientID='', userID='', artist='', track='', mood='', era='', genre='', popularity='', similarity='',
                count='10'):
    """
    ...
    query.addQueryOption('SELECT_DETAIL',
...
def radioEvent(clientID='', userID='', radioID='', gnID='', event='TRACK_PLAYED', count='10', popularity='',
               similarity=''):
    if clientID == '' or userID == '':
        print('ClientID and UserID are required')
        return None
...
def search(clientID='', userID='', artist='', album='', track='', toc=''):
    ...
    if responseElem.attrib['STATUS'] == 'OK':
        ...
        if artistOriginElem is not None:
            ...
            metadata['artist_origin'], metadata['artist_era'], metadata['artist_type'] = _getOET(clientID, userID,
                                                                                                 metadata['album_gnid'])
        ...
        for albumElem in albums:
            if albumElem.attrib["ORD"] == str(number):
                ...
                if artistOriginElem is not None:
                    ...
                    # Try to get OET again by fetching album by GNID
                    metadata['artist_origin'], metadata['artist_era'], metadata['artist_type'] = _getOET(clientID,
                                                                                                         userID,
                                                                                                         metadata[
                                                                                                             'album_gnid'])
...
def get_discography(clientID='', userID='', artist='', rangeStart=1, rangeEnd=10):
    ...
    query.addQueryOption('SELECT_EXTENDED', 'COVER,REVIEW,ARTIST_BIOGRAPHY,ARTIST_IMAGE,ARTIST_OET,MOOD,TEMPO')
    query.addQueryOption('SELECT_DETAIL',
                         'GENRE:3LEVEL,MOOD:2LEVEL,TEMPO:3LEVEL,ARTIST_ORIGIN:4LEVEL,'
                         'ARTIST_ERA:2LEVEL,ARTIST_TYPE:2LEVEL')
